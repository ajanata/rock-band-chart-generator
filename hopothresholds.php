<?php

    $HOPOS["RB"]["areyougonnabemygirl"] = 250;
    $HOPOS["RB"]["ballroomblitz"] = 250;
    $HOPOS["RB"]["blackholesun"] = 95;
    $HOPOS["RB"]["brainpower"] = 250;
    $HOPOS["RB"]["daylatedollarshort"] = 250;
    $HOPOS["RB"]["deadonarrival"] = 250;
    $HOPOS["RB"]["gowiththeflow"] = 250;
    $HOPOS["RB"]["greengrass"] = 250;
    $HOPOS["RB"]["highwaystar"] = 250;
    $HOPOS["RB"]["igetby"] = 250;
    $HOPOS["RB"]["imsosick"] = 92;
    $HOPOS["RB"]["paranoid"] = 250;
    $HOPOS["RB"]["welcomehome"] = 95;
    $HOPOS["RB"]["alabamagetaway"] = 250;
    $HOPOS["RB"]["bodhisattva"] = 250;
    $HOPOS["RB"]["ramblinman"] = 250;
    $HOPOS["RB"]["action"] = 250;
    # this is in the wrong spot in the song's .dta so it isn't processed
    # rb3 updates this to be correct so I'll indicate it now
    $HOPOS["RB"]["aestheticsofhate"] = 250;
    $HOPOS["RB"]["badtothebone"] = 90;
    $HOPOS["RB"]["blindedbyfear"] = 250;
    $HOPOS["RB"]["bloodsugarsexmagik"] = 90;
    $HOPOS["RB"]["completecontrol"] = 250;
    $HOPOS["RB"]["elscorcho"] = 100;
    $HOPOS["RB"]["everfalleninlove"] = 250;
    $HOPOS["RB"]["kissthemforme"] = 90;
    $HOPOS["RB"]["koolthing"] = 250;
    $HOPOS["RB"]["lasttraintoclarksville"] = 250;
    $HOPOS["RB"]["melatonin"] = 90;
    $HOPOS["RB"]["monkeygonetoheaven"] = 250;
    $HOPOS["RB"]["numberofthebeast"] = 250;
    $HOPOS["RB"]["ridingonthewind"] = 250;
    $HOPOS["RB"]["rockandrollstar"] = 250;
    $HOPOS["RB"]["screamingforvengeance"] = 250;
    $HOPOS["RB"]["shockwave"] = 250;
    $HOPOS["RB"]["silver"] = 85;
    $HOPOS["RB"]["simpleman"] = 95;
    $HOPOS["RB"]["snowheyoh"] = 80;
    $HOPOS["RB"]["synchronicity2"] = 250;
    $HOPOS["RB"]["thepowerofequality"] = 90;
    $HOPOS["RB"]["thrasher"] = 250;
    $HOPOS["RB"]["toxicity"] = 95;
    $HOPOS["RB"]["twoweeks"] = 250;
    $HOPOS["RB"]["withoutyou"] = 100;
    
    # AC/DC
    # this song has a different internal name...
    $HOPOS["RB"]["thejack_live"] = 95;
    $HOPOS["RB"]["thejack_live 360"] = 95;
    
    # bonus DLC
    $HOPOS["RB"]["bounce"] = 90;
    $HOPOS["RB"]["getiton"] = 125;
    $HOPOS["RB"]["prequeltothesequel"] = 125;
    $HOPOS["RB"]["thefeeling"] = 125;
    $HOPOS["RB"]["thetimeiswrong"] = 125;
    $HOPOS["RB"]["young"] = 70;
    
    # 11/11 foo fighters
    $HOPOS["RB"]["doll"] = 90;
    $HOPOS["RB"]["februarystars"] = 90;
    $HOPOS["RB"]["heyjohnnypark"] = 90;
    $HOPOS["RB"]["myhero"] = 90;
    
    # 11/18
    $HOPOS["RB"]["gone"] = 90;
    $HOPOS["RB"]["swamped"] = 90;
    
    # 11/25
    $HOPOS["RB"]["jesuschristpose"] = 90;
    
    # 12/9
    $HOPOS["RB"]["dontspeak"] = 90;
    $HOPOS["RB"]["justagirl"] = 90;

    # 12/16
    $HOPOS["RB"]["hillbillydeluxe"] = 90;
    
    # 12/30
    $HOPOS["RB"]["hymn43"] = 90;
    
    # 2009
    # 1/6
    $HOPOS["RB"]["oobydooby"] = 250;
    
    # 1/13
    $HOPOS["RB"]["letloverule"] = 90;

    # 3/3
    $HOPOS["RB"]["lenny"] = 90;
    
    # 3/10, 3/17, 3/24
    $HOPOS["RB"]["garden"] = 90;
    $HOPOS["RB"]["lastresort"] = 90;
    
    # 4/14 and 4/21
    $HOPOS["RB"]["catcherintherye"] = 90;
    $HOPOS["RB"]["therewasatime"] = 90;
    $HOPOS["RB"]["takeitontherun"] = 70;
    
    # 4/28
    $HOPOS["RB"]["tedjustadmitit"] = 70;
    
    # 5/12 5/19 5/26 6/2
    $HOPOS["RB"]["storyofmylife"] = 250;
    $HOPOS["RB"]["stricken"] = 70;
    
    $HOPOS["RB"]["sweettalk"] = 90;
    
    # july
    $HOPOS["RB"]["crawl"] = 90;
    $HOPOS["RB"]["whatahorriblenight"] = 250;
    
    # august
    $HOPOS["RB"]["lonelyasyou"] = 90;

    # 9/1 9/8
    $HOPOS["RB"]["guerrillaradio"] = 90;
    $HOPOS["RB"]["originofthespecies"] = 90;
    
    # 9/8 9/15 9/22 9/29 10/6
    $HOPOS["RB"]["would"] = 90;
    $HOPOS["RB"]["handmedownworld"] = 90;
    $HOPOS["RB"]["headoverfeet"] = 90;
    
    # 10/13 10/20 abbey road
    $HOPOS["RB"]["somebodytolove"] = 90;
    
    # 10/27 11/3 LEGO Rock Band
    $HOPOS["RB"]["dreamingofyou"] = 250;
    $HOPOS["RB"]["wearethechampions"] = 90;
    $HOPOS["RB"]["valerie"] = 90;
    $HOPOS["RB"]["thunder"] = 90;
    $HOPOS["RB"]["dig"] = 90;
    
   	# 11/10 11/17 11/24 12/1 12/8
	$HOPOS["RB"]["theleavingpt2"] = 90;
	$HOPOS["RB"]["beautifulthieves"] = 90;
	
	# 12/ 15 12/22 Country Track Pack exclusives
	$HOPOS["RB"]["buildabridge"] = 90;
	
	# 2010
	
	# 1/5
	$HOPOS["RB"]["bandontherunlive"] = 90;

	# 2/16
	$HOPOS["RB"]["imsickyall"] = 90;

	# march
	$HOPOS["RB"]["youdontknowwhatloveis"] = 90;

	$HOPOS["RB"]["hungrywolf"] = 90;

	$HOPOS["RB"]["upfromtheskies"] = 90;
	$HOPOS["RB"]["littlewing"] = 90;
	$HOPOS["RB"]["castlesmadeofsand"] = 90;

	# april/may
	$HOPOS["RB"]["again"] = 90;
	$HOPOS["RB"]["theclimb"] = 90;

	# may/june
	$HOPOS["RB"]["alivepod"] = 90;
	$HOPOS["RB"]["throwingstones"] = 90;

	# GDRB
	$HOPOS["RB"]["whenicomearound"] = 90;
	$HOPOS["RB"]["goodriddance"] = 90;
	$HOPOS["RB"]["novacaineshesarebel"] = 90;
	$HOPOS["RB"]["lastnightonearth"] = 90;
	$HOPOS["RB"]["littlegirl"] = 90;

	# june/july
	$HOPOS["RB"]["thisafternoon"] = 90;
	$HOPOS["RB"]["figuredyouout"] = 90;
	$HOPOS["RB"]["photograph"] = 90;
	$HOPOS["RB"]["nomoretears"] = 90;

	# july/august
	$HOPOS["RB"]["igotmine"] = 90;

	$HOPOS["RB"]["heyjoelive"] = 90;
	$HOPOS["RB"]["areyouexperienced"] = 90;
	$HOPOS["RB"]["deadandbloated"] = 90;

	# rb3
	$HOPOS["RB"]["yoshimibattles"] = 90;

	$HOPOS["RB"]["blazeofglory"] = "100";
	$HOPOS["RB"]["folsomprisonblues"] = "90";
	$HOPOS["RB"]["loveme2x"] = "120";
	$HOPOS["RB"]["roadhouseblues"] = "90";
	$HOPOS["RB"]["wearethechampions2"] = "90";

	$HOPOS["RB"]["rudiecantfail"] = 90;

?>
