...
function makeChart($notetracks, $measures_all, $timetrack, $events_all, $vocals, $diff, $game,
	...
	$instruments
		= ($do_vocals ? "vocals " : "")
		. ($do_guitar ? "guitar: " . $diff["guitar"] . " " : "")
		. ($do_bass ? "bass: " . $diff["bass"] . " " : "")
		. ($do_drums ? "drums: " . $diff["drums"] : "");
...
// ugly hack
	...
	imagestring($im, 3, 0, $HEIGHT - 27, "http://charts.ajanata.com/" . strtolower($game) . "/", $gray);
    ...
    if (DRAWPLAYERLINES) {
        imagestring($im, 3, WIDTH-120, 15 + SHOWFORCED*15, "Player 1", $player1);
        imagestring($im, 3, WIDTH-60, 15 + SHOWFORCD*15, "Player 2", $player2);
    }
	...
	foreach($measures_all["guitar"] as $index => &$meas) {
	   ...
	   if ($x + PXPERBEAT * $meas["num"] * 4 / $meas["denom"] > WIDTH - 25) {
        ...
        if ($do_guitar) {
            ...
            drawMeasureBackground($im, $x, $y, $meas, $events_all["guitar"], $sections, "guitar", $diff["guitar"], $game);
            drawMeasureNotes($im, $x, $y, $meas, $notetracks["guitar"][$diff["guitar"]], $game, "guitar", $diff["guitar"]);
            drawMeasureScores($im, $x, $y + STAFFHEIGHT*(4-($game == "GHOT")), $meas, $diff["guitar"]);
            
            $y += (5 - ($game == "GHOT")) * STAFFHEIGHT + 45;
            
            $measScore += 4 * $measures_all["guitar"][$index]["mscore"][$diff["guitar"]];
        }
        if ($do_bass && $game != "GH1") {
            // draw bass measure
            drawMeasureBackground($im, $x, $y, $measures_all["bass"][$index], $events_all["bass"], $sections, "bass", $diff["bass"], $game);
            drawMeasureNotes($im, $x, $y, $measures_all["bass"][$index], $notetracks["bass"][$diff["bass"]], $game, "bass", $diff["bass"]);
            drawMeasureScores($im, $x, $y + STAFFHEIGHT*(4-($game == "GHOT")), $measures_all["bass"][$index], $diff["bass"]);
                                    
            $y += (5 - ($game == "GHOT")) * STAFFHEIGHT + 45;
            
            $measScore += ($game == "RB" ? 6 : 4) * $measures_all["bass"][$index]["mscore"][$diff["bass"]];
        }
        if ($do_drums && ($game == "RB" || $game == "TBRB")) {
            // draw drums measure
           drawMeasureBackground($im, $x, $y, $measures_all["drums"][$index], $events_all["drums"], $sections, /*"drums"*/ "drums", $diff["drums"], $game);
            drawMeasureNotes($im, $x, $y, $measures_all["drums"][$index], $notetracks["drums"][$diff["drums"]], $game, /*"drums"*/ "drums", $diff["drums"]);
            drawMeasureScores($im, $x, $y + STAFFHEIGHT*3, $measures_all["drums"][$index], $diff["drums"]);
    
            $y += 4 * STAFFHEIGHT + 45;
            
            $measScore += 4 * $measures_all["drums"][$index]["mscore"][$diff["drums"]];
        }
