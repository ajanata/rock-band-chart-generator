...
<ul>
<li>Fixed some charts ending early (specifically, Float On was ending 3 measures early). I was always using the 'end' event from the guitar track, but on some songs that is too early (again, see Float On). I changed it to use the latest end event from all 4 tracks. Note that the only old chart that may have been affected by this that has been fixed is Float On: I'm not re-doing almost 500 songs when only a handful are affected. If you find a chart that ends early, please let me know so I can fix it. Credit to Kawigi for pointing this out.</li>
<li>I have decided to make the <a href="http://ajanata.com/projects/phpspopt/log">version tracking log public</a>. These are the raw comments that I put on the changes I make when I commit them to my version tracking repository. Most people will find no point in looking at them, and those that do will probably be bored -- but it's an interesting insight into how this project has evolved in the past 8 or 9 months.</li>
</ul></p>
